...
@pytest.mark.asyncio
async def test_collection_consistency_route_returns_404_when_inaccessible(
    client, db, seeded_collection, seeded_package
):
    """GET /collections/{id}/consistency returns 404 when the user cannot access the package."""
    from src.auth import CurrentUser, get_accessible_package_ids, get_current_user
    from src.main import app
    ...
    resp = await client.get(f"/api/v1/collections/{seeded_collection.id}/consistency")
    app.dependency_overrides.pop(get_current_user, None)
    app.dependency_overrides.pop(get_accessible_package_ids, None)
...
async def test_dataset_delete_returns_404_when_inaccessible(client, db, bare_dataset):
    ...
    resp = await client.delete(f"/api/v1/datasets/{bare_dataset.id}")
    app.dependency_overrides.pop(get_current_user, None)
    app.dependency_overrides.pop(get_accessible_package_ids, None)
