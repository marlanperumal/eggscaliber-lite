...
def get_dataset(dataset_id: int, session: Session = Depends(get_session)):
    try:
        return dataset_service.get_with_fields(session, dataset_id)
...
def get_dataset_responses(
    ...
    try:
        return dataset_service.get_responses(session, dataset_id, page, page_size)
    except DatasetNotFoundError:
        raise HTTPException(status_code=404, detail="Dataset not found") from None
...
def get_field_tree(dataset_id: int, session: Session = Depends(get_session)):
    try:
        return analytics_service.get_field_tree(session, dataset_id)
    except DatasetNotFoundError:
        raise HTTPException(status_code=404, detail="Dataset not found") from None
...
def get_weight_fields(dataset_id: int, session: Session = Depends(get_session)):
    try:
        return analytics_service.get_weight_fields(session, dataset_id)
    except DatasetNotFoundError:
        raise HTTPException(status_code=404, detail="Dataset not found") from None
