...
| **Sentry** | https://sentry.io | Two projects: `eggscaliber-api` (FastAPI) → `SENTRY_DSN`, `SENTRY_ORG`, `SENTRY_PROJECT`; `eggscaliber-web` (Next.js) → `NEXT_PUBLIC_SENTRY_DSN` |
| **PostHog** | https://app.posthog.com | `NEXT_PUBLIC_POSTHOG_KEY` |
...
| Platform | Steps |
|---|---|
| **Vercel** | New Project → import GitHub repo → Root Directory: `apps/web` → add all `NEXT_PUBLIC_*` + `CLERK_SECRET_KEY` env vars |
